...
> 基于 `requirement.md` 的五阶段计划，细化为可执行的 Step。每个 Step 包含目标、产物、关键实现要点、验证方式。
...
**目标**：前端工程跑通，实现登录注册页，与后端联调，并落地炫酷现代年轻风格的视觉基调。

**产物**
- `frontend/` Vue/React 工程
- 登录页、注册页、API client 封装
- 全局主题样式（深色霓虹 + 毛玻璃 + 渐变流光）
...
5. **落地炫酷现代年轻风格**（参照 `architecture.md` 界面风格设计规范）：
   - 配置 Tailwind 扩展调色板：深空黑/午夜蓝主色 + 霓虹紫/电光蓝/赛博粉/辉光青点缀色。
   - 引入字体：标题用 Montserrat/HarmonyOS Sans 粗体，正文无衬线。
   - 引入图标库 Lucide / Phosphor Icons（线性霓虹风）。
   - 全局基础样式：深色背景 + 毛玻璃工具类（`backdrop-blur` + 半透明 + 细边框）。
   - 登录卡片采用毛玻璃质感 + 外发光；按钮用霓虹渐变 + 悬停发光放大微交互（可用 Framer Motion）。
   - 预留 `ParticleBg` 粒子背景组件接入位（本步可先用静态渐变占位，Step 2.5 接入真实粒子）。

**验证**
- 浏览器打开前端，完成注册→登录→跳转首页；Vite 代理 `/api` 到后端 8000。
- 登录页呈现深色霓虹 + 毛玻璃卡片的炫酷风格，按钮悬停有发光放大反馈。
...
**目标**：前端展示情绪雷达图与实时粒子背景，落地炫酷现代年轻风格的视觉呈现。
...
**实现要点**
1. `GET /api/songs/{id}/radar` 返回 7 维数据（响度、高频、人声、节奏、声场、空间、层次）+ 情绪主色调。
2. `EmotionRadar`：p5.js 绘制 7 边形雷达图，数值动画过渡；采用霓虹描边 + 渐变填充 + 发光节点，节点带 `shadowBlur` 外发光。
3. `ParticleBg`：根据情绪主色调生成粒子，悲伤冷色（蓝/紫）、欢快暖色（橙/黄）、热血红橙、宁静青绿。监听 `windowResized` 重设画布；粒子带拖尾与辉光效果，形成"会呼吸的动态封面"。
4. `SongDetail`：组合雷达图 + 粒子背景 + MusicKit 播放按钮；播放时背景随情绪变化。卡片面板用毛玻璃质感，标题/按钮用渐变流光。
5. 移动端降级：通过 `window.innerWidth` 判断，移动端粒子数降至 60、关闭重模糊以保帧率。

**验证**
- 打开歌曲详情页，播放歌曲后雷达图与粒子背景正确呈现并随情绪变色，整体呈现深色霓虹炫酷风格。
...
**实现要点**
1. 情绪日记：用户每日记录心情 + 关联歌曲，`emotion_diary` 表。
2. 每周报告：聚合本周听歌情绪分布，生成雷达图趋势。
3. 更新 README：安装、启动、架构说明、截图。
4. 原"收藏"功能演进为"点赞"互动，完整实现见第五阶段（Step 5.4）。

**验证**
- 完整走通：授权→听歌→情绪画像→每周报告，全流程无报错。

---

# 第五阶段：用户分享与互动（第 7 周）

> 对应 `requirement.md`「四、用户分享与互动功能设计」：分享、推荐列表、点赞互动、用户主页。

## Step 5.1 分享数据模型与分享 API

**目标**：打通"AI 分析 → 分享 → 进入他人推荐列表"链路。

**产物**
- `shares` / `likes` 表（`app/models/share.py`）
- `app/api/shares.py`、`app/services/share.py`

**实现要点**
1. 新表 `shares`：`user_id`（分享者）、`song_id`（专辑/播放列表）、`platform`（apple/netease）、`comment`（分享语，可选）、`created_at`。
2. 新表 `likes`：`share_id`、`user_id`，`UNIQUE(user_id, share_id)` 每人每条分享只赞一次。
3. `POST /api/shares`：仅当该歌曲已有 AI 分析结果（`ai_predictions` 非空）才允许分享，否则 400"请先完成 AI 情绪分析"。
4. 平台标记：从 `songs.platform` 记录分享来源，供推荐端平台过滤使用。

**验证**
- 对未分析的歌曲分享返回提示；对已分析的专辑/播放列表分享成功，记录入库。

---

## Step 5.2 推荐列表服务（情绪相近优先）

**目标**：推荐列表完全来自其他用户的分享，按口味相近优先排序，无匹配随机补足。

**产物**
- `GET /api/recommend` 端点（`app/api/recommend.py`）
- `app/services/recommend.py` 推荐流水线

**实现要点**
1. 拉取当前用户之外所有用户的分享。
2. **平台过滤**：仅返回用户已绑定平台（`users` 的 apple/netease 绑定状态）对应的分享。
3. **情绪相似度**：聚合当前用户 AI 分析结果的平均 7 维情绪向量，与各分享内容的 7 维向量计算余弦相似度，降序。
4. **点赞加权**：相似度排序基础上，点赞数越多排名越靠前（加权合并得分）。
5. **随机兜底**：不足最大列表长度（如 20 条）时，用随机分享补齐，保证列表不空。

**验证**
- 构造多个口味不同的测试账号：验证优先推荐相似分享、无匹配时随机补足、平台过滤生效。

---

## Step 5.3 用户主页（分享者歌单）

**目标**：推荐卡片展示分享者，点击可查看其全部歌单。

**产物**
- `GET /api/users/{id}/songs`（`app/api/users.py`）
- 前端 `UserProfile.tsx`

**实现要点**
1. 后端：返回指定用户的歌曲列表（其同步/分析的专辑与播放列表）。
2. 推荐卡片渲染分享者用户名（"由 @xxx 分享"）。
3. 点击用户名跳转 `/users/{id}`，展示该用户全部歌单。

**验证**
- 点击推荐卡片上的分享者，进入其主页看到完整歌单。
...
**产物**
- `POST/DELETE /api/shares/{id}/like`
- 前端 `ShareFeed.tsx`、`ShareCard.tsx`

**实现要点**
1. 点赞接口：联合唯一约束保证不重复，可赞可取消。
2. `ShareCard`：专辑信息、AI 情绪徽章、分享者用户名、点赞数（❤️）。
3. `ShareFeed`：首页推荐列表展示分享流，替代原系统推荐位；点赞/取消即时刷新并影响排序。

**验证**
- 点赞/取消即时刷新；点赞数变化影响 feed 排序；端到端无报错。
...
- [ ] Step 4.3 性能优化验证
- [ ] Step 4.4 扩展功能与文档收尾
- [x] Step 5.1 分享模型与分享 API 可用
- [x] Step 5.2 推荐列表按情绪相似度排序、平台过滤、随机兜底
- [x] Step 5.3 用户主页歌单展示
- [x] Step 5.4 点赞互动与推荐页上线
